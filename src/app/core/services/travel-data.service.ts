...
export class TravelDataService {
  ...
  public createTravel(data: any): void{
    const travel = new Travel(this.nextId.toString(), data.name, data.description, data.opinion, data.startingDate, data.endingDate, data.favoritePlate);
  ...
  public updateTravel(id: string, data: any): void {
    const index = this.travels.findIndex((elem) => elem.id === id);
    const travel = new Travel(id, data.name, data.description, data.opinion, data.startingDate, data.endingDate, data.favoritePlate);
